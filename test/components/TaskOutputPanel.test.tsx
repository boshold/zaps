...
describe("TaskOutputPanel", () => {
  it("renders lines", () => {
    const { lastFrame } = render(<TaskOutputPanel lines={["hello", "world"]} visibleLines={10} />);
  ...
  it("replaces non-printable chars with spaces", () => {
    const { lastFrame } = render(<TaskOutputPanel lines={["hello\x01world"]} visibleLines={10} />);
    const frame = lastFrame() ?? "";
  ...
  it("limits visible lines from the end", () => {
    ...
    const { lastFrame } = render(<TaskOutputPanel lines={lines} visibleLines={2} />);
    const frame = lastFrame() ?? "";
