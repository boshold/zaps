...
describe("LogView", () => {
  it("renders captured lines", () => {
    ...
    const { lastFrame } = render(
      <LogView serviceName="api" lines={lines} autoScroll={true} offset={0} />,
    );
  ...
  it("renders service name in header", () => {
    const { lastFrame } = render(
      <LogView serviceName="my-service" lines={[]} autoScroll={true} offset={0} />,
    );
    const frame = lastFrame() ?? "";
  ...
  it("renders scroll help bar", () => {
    const { lastFrame } = render(
      <LogView serviceName="api" lines={[]} autoScroll={true} offset={0} />,
    );
    const frame = lastFrame() ?? "";
